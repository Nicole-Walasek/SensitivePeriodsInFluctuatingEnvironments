...
def policyPlotReducedMerge(r, minProb, lines, cueValidityArr, T, adultT, levelsAutoCorrToPlot, autoCorrDict,
    ...
    for symmArg, autoCorrCurr in rowVec:  # one column per adultT
        ...
        for cueVal in cueValidityArr:  # one row per cue reliability value
            ...
            for t in tValues:
                ...
                circles(aggregatedResultsDF['time'], aggregatedResultsDF['newpE1'], s=area, ax=ax, c=colors, zorder=2,
                        lw=0.5)
                if adulthood:
                    if t == ontT:
            ...
            if lines:
                ...
                for t in np.arange(0, T - 1, 1):
                    ...
                    nextIdent = zip(list(subDF2.x0), list(subDF2.x1))
                    currIdent = zip(list(subDF1.x0), list(subDF1.x1))
                    ...
                    for subIDX in range(len(subDF1)):
                        yArr = [[subDF1['newpE1'].loc[subIDX], subDF2['newpE1'].loc[yIDX]] for yIDX in
                                yvalsIDXAll[subIDX]]
                        [ax.plot(timeArr, yArrr, ls='solid', marker=" ", color='#e6daa6', zorder=1, lw=1.5) for yArrr in
                         yArr] ##888888
                    del yArr
    ...
    fig.text(0.08, 0.42, 'estimate', fontsize=25, horizontalalignment='left', verticalalignment='center',
             transform=ax.transAxes, rotation='vertical')
    autoCorrCoord = 0.9
    ...
    for figCoord, adultT in zip(figVals, newMergeArr):
        ...
        plt.savefig(os.path.join(twinResultsAggregatedPath, 'DevelopmentalTrajectoryReducedMerge.png'), bbox_inches='tight' ,dpi=1000)
...
def policyPlotReducedMergeBW(r, minProb, lines, cueValidityArr, T, adultT, levelsAutoCorrToPlot, autoCorrDict,
    ...
    for symmArg, autoCorrCurr in rowVec:  # one column per adultT
        ...
        for cueVal in cueValidityArr:  # one row per cue reliability value
            ...
            for t in tValues:
                ...
                # now plot the developmental trajectories
                circles(aggregatedResultsDF['time'], aggregatedResultsDF['newpE1'], s=area, ax=ax, c=colors, zorder=3,
                        lw=0.5)
                if adulthood:
                    if t == ontT:
                        lastPE1 = aggregatedResultsDF['pE1']
            ...
            if lines:
                ...
                for t in np.arange(0, T - 1, 1):
                    ...
                    for subIDX in range(len(subDF1)):
                        yArr = [[subDF1['newpE1'].loc[subIDX], subDF2['newpE1'].loc[yIDX]] for yIDX in
                                yvalsIDXAll[subIDX]]
                        [ax.plot(timeArr, yArrr, ls='solid', marker=" ", color='#D3D3D3', zorder=1, lw=1.5) for yArrr in
                         yArr] ##888888
                    del yArr
                elapsedTime = time.clock() - startTime
                print "Elapsed time plotting the lines: " + str(elapsedTime)
            ...
            radii = []
            for idx in range(len(coordinates)):
                colorsPies = ['black' for idj in decisionsPies[idx]]
            ...
            plt.plot(np.arange(0, ontT +adultT +1, 1), [prior] * len(np.arange(0,  ontT +adultT +1, 1)),
                     zorder=2, color='#bd0a06', lw=2, ls='solid')
            ...
            if adulthood:
                ...
                for idxCurr, pE1 in enumerate(lastPE1):
                    ...
                    ax.plot(xVec, yVec, ls='solid', marker=" ", color='#D3D3D3', zorder=1, lw=1.5) #D3D3D3
                ...
                circles(adultTimeVec, posVecNewAdult, s=area, ax=ax, c='black', zorder=3, lw=0.5)
            ...
            plt.plot([ontT] * len(np.arange(1, convertVal + 0.2, 0.1)), np.arange(1, convertVal + 0.2, 0.1),
                     zorder=-1, color='#bd0a06', lw=2, ls='-')  # 696969

            plt.plot([ontT + 1] * len(np.arange(1, convertVal + 0.2, 0.1)), np.arange(1, convertVal + 0.2, 0.1),
                     zorder=-1, color='#bd0a06', lw=2, ls='--') #696969

            plt.plot([ontT+5]*len(np.arange(1, convertVal + 0.2, 0.1)), np.arange(1, convertVal + 0.2, 0.1),zorder = -1, color = '#bd0a06', lw = 2,ls ='--')
            plt.plot([ontT + 10] * len(np.arange(1, convertVal + 0.2, 0.1)), np.arange(1, convertVal + 0.2, 0.1),
                     zorder=-1, color='#bd0a06', lw=2, ls='--')
            ...
            if ix == len(cueValidityArr) - 1 and jx == 0:
                ...
                plt.xticks([11, 15, 20, 30], [1, 5, 10, 20], fontsize=20)
            else:
                plt.xticks([])
                ax.get_xaxis().set_visible(False)
    ...
    plt.subplots_adjust(wspace=0.2, hspace=0.2, bottom=0.075, top=top)
    ...
    for figCoord, adultT in zip(figVals, newMergeArr):
        ...
        # plt.savefig(os.path.join(twinResultsAggregatedPath,'DevelopmentalTrajectoryReduced_%s.png' %(startEnv)), dpi = 400)
        plt.savefig(os.path.join(twinResultsAggregatedPath, 'DevelopmentalTrajectoryReducedMergeBW.png'),bbox_inches='tight' ,dpi=1000)
...
def policyPlotReducedOverview2(r, minProb, lines, cueValidityArr, T, adultTArr,levelsAutoCorrToPlot , autoCorrDict,
    ...
    for adultT, autoCorr in rowVec:  # one column per adultT
        ...
        for cueVal in cueValidityArr:  # one row per cue reliability value
            ...
            if (jx+1) %len(autoCorrValSubset) == 0 and not jx == (len(adultTArr)*len(autoCorrValSubset))-1:
                ...
                ax.vlines([paramVLine], -0.03, 1.03, transform=ax.get_xaxis_transform(),color='black', lw=2,clip_on=False)
    ...
    if len(adultTArr) ==3:
        ...
        fig.text(0.95, 0.38, 'cue reliability', fontsize=25, horizontalalignment='right', verticalalignment='center',
                 transform=ax.transAxes, rotation='vertical')
        fig.text(0.08, 0.38, 'estimate', fontsize=25, horizontalalignment='left', verticalalignment='center',
                 transform=ax.transAxes, rotation='vertical')
        autoCorrCoord = 0.85
        ...
        fig.text(0.98, 0.42, 'cue reliability', fontsize=25, horizontalalignment='right', verticalalignment='center',
                 transform=ax.transAxes, rotation='vertical')
        fig.text(0.05, 0.42, 'estimate', fontsize=25, horizontalalignment='left', verticalalignment='center',
                 transform=ax.transAxes, rotation='vertical')
        autoCorrCoord = 0.9
    else:
        top = 0.8
    ...
    for figCoord, adultT in  zip(figVals, adultTArr):
        ...
        if len(figVals) ==2:
            ...
            if figCoord <0.3:
                ...
                figCoordF = figCoord-0.055
        fig.text(figCoordF,autoCorrCoord,'%s\n\nautocorrelation' %adultT, fontsize = 25, horizontalalignment = 'center', verticalalignment = 'center', transform=ax.transAxes, rotation = 'horizontal')


        plt.savefig(os.path.join(twinResultsAggregatedPath, 'DevelopmentalTrajectoryReduced.png'), bbox_inches='tight',dpi=1000)
...
def policyPlotReduced(T,r,markovProbabilities, pC0E0Arr, tValues, dataPath, lines, argumentR, argumentP, minProb,mainPath, plottingPath):
    ...
    for cueVal in pC0E0Arr: # for each cue validity
        ...
        for pE0E0, pE1E1 in markovProbabilities: # for each prior
            ...
            for t in tValues:
                ...
                # now plot the developmental trajectories
                circles(np.array(aggregatedResultsDF['time']),np.array(aggregatedResultsDF['newpE1']), s =np.array(aggregatedResultsDF['area']), ax = ax,c = colors, zorder = 2, lw = 0.5)
    ...
    #plt.savefig(os.path.join(resultPath, 'DevelopmentalTrajectoryReduced.jpg'), dpi=900)
